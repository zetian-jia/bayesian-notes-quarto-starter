...
| | 内容 | 说明 |
|---|---|---|
| **Q1 潜变量** | 单细胞层面 $x_{ij}\in\{0,1,\text{NA}\}$ → 位点均值  $\frac{\bar x_i}{\tilde x_i}$ → 区间收缩残差 $r_{Ij}$ → PCA 隐空间 $\mathbf{x}_j^P$ | 潜变量是**多层聚合链**：细胞×位点(稀疏0/1/NA) → 位点(跨细胞) → 区间(跨位点，单细胞) → 低维隐空间(跨区间)，逐层降维压缩稀疏信号 |
...
- **$\bar{x}_i$**：第 $i$ 个 CpG 位点的**跨细胞平均甲基化水平**（cross-cell average），是所有覆盖该位点的细胞的甲基化状态的简单算术平均

- **$C_i$**：前面定义过的集合——在位点 $i$ 上有读段覆盖的所有细胞的索引集合.(**有覆盖才算**!!没有覆盖不算)

- **$\bar{x}_i = \dfrac{1}{|C_i|}\sum_{j\in C_i} x_{ij}$**
...
> $x_{ij}$ : 甲基化为1, 非甲基化为0, 所以刚好就是数1的个数,除以 $C_j$
